...
import utils.guaranteeParsing.GuaranteeInstructionBuilder
...
class EnsureGuaranteeService @Inject() (
  ...
  def ensureGuarantee(xml: NodeSeq): ParseHandler[NodeSeq] =
    routeChecker.gbOnlyCheck(xml).flatMap { gbOnly =>
      if (gbOnly)
        Right(xml)
      else for {
        instructionSets <- parseInstructionSets(xml)
        updatedXml <- updateXml(xml, instructionSets)
      } yield updatedXml
    }

  def parseInstructionSets(xml: NodeSeq): ParseHandler[Seq[TransformInstructionSet]] =
    for {
      guarantees <- xmlReaders.parseGuarantees(xml)
      gooBlocks <- xmlReaders.gOOITEGDSNode(xml)
      blocks <- gooBlocks.toList.traverse(instructionBuilder.buildInstructionSet(_, guarantees))
    } yield blocks
