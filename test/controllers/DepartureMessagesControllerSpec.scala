...
class DepartureMessagesControllerSpec extends AnyFreeSpec with Matchers with GuiceOneAppPerSuite with OptionValues with ScalaFutures with MockitoSugar with BeforeAndAfterEach with TestXml {
  ...
  "POST /movements/departures/:departureId/messages" ignore {
    "must return Accepted when successful" in {
      ...
      val request = fakeRequestMessages(method = "POST", uri = routes.DepartureMessagesController.sendMessageDownstream("123").url, body = <CC014A></CC014A>)
      val result = route(app, request).value
    ...
    "must return BadRequest when invalid XML payload is sent" in {
      val request = fakeRequestMessages(method = "POST", uri = routes.DepartureMessagesController.sendMessageDownstream("123").url, body = <InvalidCC014A></InvalidCC014A>)
