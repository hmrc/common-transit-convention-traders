...
class ValidateDepartureMessageActionSpec
    extends AnyFreeSpec
    with Matchers
  ...
  "ValidateDepartureMessageAction" - {
    ...
    "must return BadRequest when passed in an invalid IE014  xml request " in {
      ...
      val req: FakeRequest[NodeSeq] =
        FakeRequest(method = "", uri = "", headers = FakeHeaders(Seq(HeaderNames.CONTENT_TYPE -> "application/xml")), InvalidCC014A)
      ...
      val expectedResult = Json.obj("message" -> expectedMessage)
    ...
    "must return BadRequest when passed in an empty request" in {
      ...
      val expectedResult = Json.obj("message" -> XmlError.RequestBodyEmptyMessage)
    ...
    "must return BadRequest when passed in incorrect request body" in {
      ...
      val expectedResult = Json.obj("message" -> XmlError.RequestBodyInvalidTypeMessage)
    ...
    "must return NotImplemented when passed in an incorrect XML request" in {
      ...
      val req: FakeRequest[NodeSeq] =
        FakeRequest(method = "", uri = "", headers = FakeHeaders(Seq(HeaderNames.CONTENT_TYPE -> "application/xml")), <CC011A></CC011A>)
