...
class AuthNewEnrolmentActionSpec extends AnyFreeSpec with Matchers with MockitoSugar with BeforeAndAfterEach {
  ...
  class Harness(authAction: AuthNewEnrolmentAction) {
  ...
  "must execute the block" - {
    "when the user is logged in and has the new enrolment" in {
      ...
      val authAction = new AuthNewEnrolmentAction(authConnector, bodyParser)
