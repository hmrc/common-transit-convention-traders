...
class ResponseArrivalSpec extends FreeSpec with MustMatchers with GuiceOneAppPerSuite with OptionValues with ScalaFutures with MockitoSugar with BeforeAndAfterEach {
  ...
  "ResponseArrival" - {
    ...
    "must have valid public messages location" in {
      ...
      result.messages mustBe routes.ArrivalMessagesController.getArrivalMessages("3").urlWithContext().url
    }
  }
