...
class ConversionConnectorSpec
  ...
  lazy val messageType           = MessageType.IE015
  ...
  "POST /messages/:messageType " - {
    "when making a successful submission, must return successful" in {
      ...
      whenReady(sut.post(MessageType.IE015, jsonStream, jsonToXml)) {
        _.reduce(_ ++ _)
          .map(_.utf8String)
          .runWith(Sink.last)
          .map {
    ...
    "when making an unsuccessful submission must return an error response" in {
      ...
      val postResponse = sut.post(MessageType.IE015, jsonStream, jsonToXml)
