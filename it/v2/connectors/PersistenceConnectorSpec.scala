...
class PersistenceConnectorSpec
  ...
  val defaultFilterParams = "?pageNumber=0&itemCount=25"
  ...
  "GET /traders/:eori/movements/departure/:departureid/messages" - {
    ...
    "on successful return of message IDs when filtering by received date is applied, return a success" in {
      ...
      server.stubFor(
        get(
          urlEqualTo(targetUrlWithTime(eori, departureId, time) + "&pageNumber=0&itemCount=25")
        )
          .willReturn(
            aResponse()
              .withStatus(OK)
    ...
    "on incorrect Json, return an error" in {
      ...
      implicit val hc = HeaderCarrier()
      val r = persistenceConnector
        .getMessages(eori, MovementType.Departure, departureId, None, None, None, None)
        .map(
          _ => fail("This should have failed with a JsResult.Exception, but it succeeded")
        )
        .recover {
      ...
      (movementSummaryList, updatedSince, movementEORI, movementReferenceNumber, pageNumber, itemCount) =>
        server.stubFor(
          get(
            ...
            .withQueryParam("pageNumber", equalTo(pageNumber.value.toString))
            .withQueryParam("itemCount", equalTo(itemCount.value.toString))
            .willReturn(
              aResponse()
                .withStatus(OK)
                .withBody(
    ...
    "on success with movementEORI filter, return a list of departure IDs" in forAll(
      ...
      (movementSummaryList, updatedSince, movementEORI, movementReferenceNumber, pageNumber, itemCount) =>
        server.stubFor(
          get(
            urlEqualTo(
              targetUrl(eori) + s"?movementEORI=${movementEORI.value}&movementReferenceNumber=${movementReferenceNumber.value}&pageNumber=0&itemCount=25"
            )
          )
            .willReturn(
              aResponse()
  ...
  "GET /traders/:eori/movements/arrivals/:arrivalId/messages" - {
      ...
      (eori, arrivalId, time, messageSummary) =>
        server.stubFor(
          get(
            ...
            .withQueryParam("itemCount", equalTo("25"))
            .willReturn(
              aResponse()
                .withStatus(OK)
                .withBody(
    ...
    "on incorrect Json, return an error" in forAll(arbitrary[EORINumber], arbitrary[MovementId]) {
      (eori, arrivalId) =>
        ...
        implicit val hc = HeaderCarrier()
        val r = persistenceConnector
          .getMessages(eori, MovementType.Arrival, arrivalId, None, None, None, None)
          .map(
            _ => fail("This should have failed with a JsResult.Exception, but it succeeded")
          )
          .recover {
  ...
  "GET /traders/:EORI/movements/arrivals" - {
    ...
    "on success with PageNumber filter, return a list of arrivals" in forAll(
      ...
      (movementSummaryList, updatedSince, movementEORI, movementReferenceNumber, pageNumber, itemCount) =>
        server.stubFor(
          get(
            urlEqualTo(targetUrl(eori) + s"?pageNumber=${pageNumber.getOrElse(PageNumber(0)).value}&itemCount=${itemCount.getOrElse(ItemCount(25)).value}")
          )
            .willReturn(
              aResponse()
                .withStatus(OK)
